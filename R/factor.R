...
summary_f <- function(x, as.factor=FALSE, ..., factor...=NULL)
{
  ...
  factorArgs <- modifyList(factorArgs, factor...)
...
order_levels <- function(x, ..., index=FALSE, frequency=TRUE, decreasing=TRUE, other="other", other_last=TRUE, exclude=NA, add_na=FALSE, ifany=TRUE, by_name=FALSE)
{
  ...
  hasNaLevel <- F; if (any(is.na(l))) hasNaLevel <- T
...
#' @export
merge_levels <- function(x, new_levels, ..., other_level="other", drop_levels=FALSE, label=NULL, sort=FALSE)
{
  # 'x' is a factor variable or a variable that can be coerced into a factor.
...
#' @export
rename_levels <- function(x, ..., in_order=FALSE, merge...=NULL, order...=NULL, label=NULL)
{
  # 'x' is a factor variable or a variable that can be coerced into a factor.
  ...
  if (!is.null(order...)) {
    ...
    orderArgs <- modifyList(orderArgs, order...)
...
cut2q <- function(v, cuts, trim=FALSE, fmt="%1.1f", dash="--", ...)
{
  ...
  attributes(f) <- modifyList(attributes(f), att) # Restore object attributes.
